  ...
  for (const markdownFile of markdownFiles) {
    ...
    const isRandomTable = (tag: string | string[]): boolean | undefined => {
      if (typeof tags === 'string') {
        ...
        return (
          tags.filter((tag) => isRandomTableTag(tag, randomTableTag)).length > 0
        );
      }
    };
